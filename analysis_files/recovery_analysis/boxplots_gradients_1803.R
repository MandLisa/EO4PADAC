...
recovery_unique$mean_prec_total <- recovery_unique$mean_prec_total * 0.1
...
q1 <- quantile(recovery_unique$mean_prec_total, 1/3, na.rm = TRUE)
q2 <- quantile(recovery_unique$mean_prec_total, 2/3, na.rm = TRUE)

# Create prec_class with actual threshold values
recovery_unique <- recovery_unique %>%
  mutate(prec_class = case_when(
    mean_prec_total <= q1 ~ paste0("≤ ", round(q1, 1)), 
    mean_prec_total > q1 & mean_prec_total <= q2 ~ paste0(round(q1, 1), " - ", round(q2, 1)),
    mean_prec_total > q2 ~ paste0("> ", round(q2, 1))
  ))
...
recovery_unique_boxplot <- recovery_unique %>%
  group_by(temp_class, height_class, prec_class) %>%
  mutate(
...
all <- recovery_unique_boxplot %>%
  ...
  pivot_longer(cols = c(height_class, temp_class, prec_class),
               names_to = "category",
...
ggsave("~/eo_nas/EO4Alps/figs/boxplot_all_1803.png", plot = p1, width = 9, height = 6.5, dpi = 300)
...
# Create the boxplot
p1 <- ggplot(central, aes(x = category, y = jitter(percent_recovered, amount = 2.5), fill = class_level)) +
  geom_boxplot(width = 0.7, coef = 0.5) +
  theme_bw(base_size = 30) +
  labs(title = "",
       x = "",
       y = "") +
