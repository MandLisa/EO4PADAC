library(ggplot2)
library(tidyverse)
library(ggforce)
library(sf)
library(gridExtra)
library(purrr)
library(jcolors)
library(GGally)
library(randomForest)
library(pdp)
library(patchwork)
library(spgwr)
library(scales)
library(GWmodel)
library(sf)
library(spdep)
library(pheatmap)
library(DataEditR)
library(readr)
library(spgwr)
library(kableExtra)
library(dplyr)

recovery <- read_csv("~/eo_nas/EO4Alps/00_analysis/_recovery/recovery_GWR.csv")

# Filter the dataset and compute the new column
recovery_filt <- recovery %>%
  group_by(ID) %>%
  filter(yod < 2013) %>%
  mutate(recov_10 = ifelse(recovery_rate <= 10, 1, 0)) %>%
  ungroup()


# compute pre-disturbance mean broadleaved and mean coniferous shares and bare 
# ground shares

# Create new columns for pre-disturbance means
recovery_filt <- recovery_filt %>%
  group_by(ID) %>%  # Group by ID to calculate means within each ID
  mutate(
    pre_dist_coni = ifelse(year < yod, mean(coniferous[year < yod], na.rm = TRUE), NA),
    pre_dist_broadl = ifelse(year < yod, mean(broadleaved[year < yod], na.rm = TRUE), NA),
    post_dist_bare = ifelse(year < yod, mean(bare_ground[year > yod], na.rm = TRUE), NA)
  ) %>%
  ungroup()

# convert recovery df to sf object
recovery_sf <- st_as_sf(recovery_filt, coords = c("x", "y"), crs = 3035)

# load hexagons and recovery df
hexagons <- st_read("~/eo_nas/EO4Alps/gis/hexagons/hex_500.shp")


# just use GRID_ID for subsequent joins
hexagons_selected <- hexagons %>%
  select(GRID_ID)

# spatial join
recovery_sf <- st_join(recovery_sf, hexagons_selected, join = st_intersects)

### temporal change of percentage recovered (not necessarily needed)
recovery_sf <- recovery_sf %>%
  mutate(period = case_when(
    yod < 2000 ~ "before",
    yod >= 2000 ~ "after",
    TRUE       ~ NA_character_  # For yod == 2000 or any other case
  ))

# spatila join
recovery_sf <- st_join(recovery_sf, hexagons_selected, join = st_intersects)

# compute predist_tree cover
recovery_sf_temp <- recovery_sf %>%
  mutate(predist_TC = pre_dist_broadl + pre_dist_coni) 


### aggregate metrics per GRID_ID
recovery_sf_temporal <- recovery_sf_temp %>%
  group_by(GRID_ID) %>%
  summarize(
    # Calculations for yod < 2000
    total_observations_yod_before_2000 = sum(yod < 2000, na.rm = TRUE),
    total_recovered_yod_before_2000 = sum(recovery_10yn[yod < 2000], na.rm = TRUE),
    percent_recovered_yod_before_2000 = (total_recovered_yod_before_2000 / total_observations_yod_before_2000) * 100,
    mean_elevation_yod_before_2000 = mean(height[yod < 2000], na.rm = TRUE),
    mean_severity_yod_before_2000 = mean(severity_relative[yod < 2000], na.rm = TRUE),
    mean_VPD_yod_before_2000 = mean(VPD_yod1[yod < 2000], na.rm = TRUE),
    mean_VPD_abs_before_2000 = mean(mean_VPD10[yod < 2000], na.rm = TRUE),
    mean_temp_before_2000 = mean(mean_temp_total[yod < 2000], na.rm = TRUE),
    mean_prec_before_2000 = mean(mean_prec_total[yod < 2000], na.rm = TRUE),
    mean_predist_TC_yod_before_2000 = mean(predist_TC[yod < 2000], na.rm = TRUE),
    mean_bare_yod_before_2000 = mean(bare_ground[yod < 2000], na.rm = TRUE),
    
    # Calculations for yod >= 2000
    total_observations_yod_after_2000 = sum(yod >= 2000, na.rm = TRUE),
    total_recovered_yod_after_2000 = sum(recovery_10yn[yod >= 2000], na.rm = TRUE),
    percent_recovered_yod_after_2000 = (total_recovered_yod_after_2000 / total_observations_yod_after_2000) * 100,
    mean_elevation_yod_after_2000 = mean(height[yod >= 2000], na.rm = TRUE),
    mean_severity_yod_after_2000 = mean(severity_relative[yod >= 2000], na.rm = TRUE),
    mean_VPD_yod_after_2000 = mean(VPD_yod1[yod >= 2000], na.rm = TRUE),
    mean_VPD_abs_after_2000 = mean(mean_VPD10[yod >= 2000], na.rm = TRUE),
    mean_temp_after_2000 = mean(mean_temp_total[yod > 2000], na.rm = TRUE),
    mean_prec_after_2000 = mean(mean_prec_total[yod > 2000], na.rm = TRUE),
    mean_predist_TC_yod_after_2000 = mean(predist_TC[yod >= 2000], na.rm = TRUE),
    mean_bare_yod_after_2000 = mean(bare_ground[yod >= 2000], na.rm = TRUE),
    
    # Overall calculations
    total_observations = n(),
    total_recovered = sum(recovery_10yn, na.rm = TRUE),
    percent_recovered_overall = (total_recovered / total_observations) * 100
  ) %>%
  mutate(
    # Differences between yod < 2000 and yod >= 2000
    percent_recovered_difference = percent_recovered_yod_after_2000 - percent_recovered_yod_before_2000,
    mean_elevation_difference = mean_elevation_yod_after_2000 - mean_elevation_yod_before_2000,
    mean_severity_difference = mean_severity_yod_after_2000 - mean_severity_yod_before_2000,
    mean_VPD_difference = mean_VPD_yod_after_2000 - mean_VPD_yod_before_2000,
    mean_VPD_abs_difference = mean_VPD_abs_after_2000 - mean_VPD_abs_before_2000,
    mean_temp_difference = mean_temp_after_2000 - mean_temp_before_2000,
    mean_prec_difference = mean_prec_after_2000 - mean_prec_before_2000,
    mean_TC_difference = mean_predist_TC_yod_after_2000 - mean_predist_TC_yod_before_2000,
    mean_bare_difference = mean_bare_yod_after_2000 - mean_bare_yod_before_2000
  ) %>%
  ungroup()


# drop NAs in my predictors
recovery_sf_temporal <- recovery_sf_temporal %>%
  drop_na(percent_recovered_difference, mean_elevation_difference, 
          mean_severity_difference, mean_VPD_difference, 
          mean_TC_difference, mean_bare_difference, mean_temp_difference, mean_VPD_abs_difference,
          mean_prec_difference)


# Check for NAs in model variables (probably not neccessariy anymore)
recovery_sf_temporal %>%
  summarise(
    na_percent_recovered_diff = sum(is.na(percent_recovered_difference)),
    na_elevation_diff = sum(is.na(mean_elevation_difference)),
    na_severity_diff = sum(is.na(mean_severity_difference)),
    na_VPD_diff = sum(is.na(mean_VPD_difference)),
    na_prec_diff = sum(is.na(mean_prec_difference)),
    na_temp_diff = sum(is.na(mean_temp_difference)),
    na_TC_diff = sum(is.na(mean_TC_difference)),
    na_mean_bare = sum(is.na(mean_bare_difference))
  )

# probably not necessary anymore
recovery_sf_temporal <- recovery_sf_temporal %>%
  filter(
    !is.na(percent_recovered_difference) & 
      !is.na(mean_elevation_difference) &
      !is.na(mean_severity_difference) & 
      !is.na(mean_VPD_difference) & 
      !is.na(mean_prec_difference) & 
      !is.na(mean_temp_difference) & 
      !is.na(mean_TC_difference) &
      !is.na(mean_bare_difference)
  )

# Perform spatial join
hexagons_recov10_temporal <- st_join(hexagons_selected, recovery_sf_temporal, join = st_intersects)


#-------------------------------------------------------------------------------
### plot recovery success before/after 2000 as difference
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = percent_recovered_difference), color = "grey") +  # Map fill color to recovery %
  scale_fill_gradient2(
    low = "blue",    # Color for negative values
    mid = "white",   # Color for 0
    high = "red",    # Color for positive values
    midpoint = 0,    # Set 0 as the midpoint
    name = ""        # Remove legend title
  ) +
  labs(
    title = "",
    subtitle = "",
    fill = ""
  ) +
  theme_bw(base_size = 18)


# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_temporal_recovery.png", plot = p1, width = 9, height = 4.5, dpi = 300)


#-------------------------------------------------------------------------------
### fit the GWR model
#-------------------------------------------------------------------------------
# Convert data to Spatial format if required by GWR
hexagons_recov10_temporal_sp <- as(hexagons_recov10_temporal, "Spatial")


# with GWmodel package

# Determine bandwidth
bw_GW_temporal <- bw.gwr(
  percent_recovered_difference ~ mean_elevation_difference + 
    mean_severity_difference + 
    mean_VPD_difference + 
    mean_TC_difference + 
    mean_bare_difference +
    mean_temp_difference +
    mean_prec_difference,
  data = hexagons_recov10_temporal_sp,
  kernel = "gaussian"
)

# Fit the GWR model
gwr_GW_temporal <- gwr.basic(
  percent_recovered_difference ~ mean_elevation_difference + 
    mean_severity_difference + 
    mean_VPD_difference + 
    mean_TC_difference + 
    mean_prec_difference +
    mean_temp_difference +
    mean_bare_difference,
  data = hexagons_recov10_temporal_sp,
  bw = bw_GW_temporal, 
  kernel = "gaussian"
)

gwr_GW_temporal 


# determine degrees of freedom
dfree <- gwr_GW_temporal$GW.diagnostic$edf


# compute t-ratios from which p-values are derived
hexagons_recov10_temporal_sp$elevation.t <- gwr_GW_temporal$SDF@data$mean_elevation/gwr_GW_temporal$SDF@data$mean_elevation_SE
hexagons_recov10_temporal_sp$severity.t <- gwr_GW_temporal$SDF@data$mean_severity/gwr_GW_temporal$SDF@data$mean_severity_SE
hexagons_recov10_temporal_sp$VPD.t <- gwr_GW_temporal$SDF@data$mean_VPD_yod1/gwr_GW_temporal$SDF@data$mean_VPD_yod1_SE
hexagons_recov10_temporal_sp$prec.t <- gwr_GW_temporal$SDF@data$mean_prec_total/gwr_GW_temporal$SDF@data$mean_prec_total_SE
hexagons_recov10_temporal_sp$temp.t <- gwr_GW_temporal$SDF@data$mean_temp_total/gwr_GW_temporal$SDF@data$mean_temp_total_SE
hexagons_recov10_temporal_sp$predist_treecov.t <- gwr_GW_temporal$SDF@data$mean_pre_dist_tree_cover/gwr_GW_temporal$SDF@data$mean_pre_dist_tree_cover_SE
hexagons_recov10_temporal_sp$bare_ground.t <- gwr_GW_temporal$SDF@data$mean_bare/gwr_GW_temporal$SDF@data$mean_bare_SE

# compute p-values
hexagons_recov10_temporal_sp$elevation.p<-2*pt(-abs(hexagons_recov10_temporal_sp$elevation.t), dfree)
hexagons_recov10_temporal_sp$severity.p<-2*pt(-abs(hexagons_recov10_temporal_sp$severity.t), dfree)
hexagons_recov10_temporal_sp$VPD.p<-2*pt(-abs(hexagons_recov10_temporal_sp$VPD.t), dfree)
hexagons_recov10_temporal_sp$prec.p<-2*pt(-abs(hexagons_recov10_temporal_sp$prec.t), dfree)
hexagons_recov10_temporal_sp$temp.p<-2*pt(-abs(hexagons_recov10_temporal_sp$temp.t), dfree)
hexagons_recov10_temporal_sp$predist_treecov.p<-2*pt(-abs(hexagons_recov10_temporal_sp$predist_treecov.t), dfree)
hexagons_recov10_temporal_sp$bare_ground.p<-2*pt(-abs(hexagons_recov10_temporal_sp$bare_ground.t), dfree)

# convert hexagons_sp to sf object
hexagons_recov10_temporal_sf <- st_as_sf(hexagons_recov10_temporal_sp)


# compute Monte Carlo permutation to check whether spatial variability is 
# statistically significant

monte_carlo_results <- gwr.montecarlo(
  mean_percent_recovered ~ mean_elevation + 
    mean_severity + 
    mean_VPD_yod1 + 
    mean_prec_total +
    mean_temp_total +
    mean_pre_dist_tree_cover + 
    mean_bare,
  data = hhexagons_recov10_temporal_sp,
  bw = bw, 
  kernel = "gaussian"
)

print(monte_carlo_results)


#-------------------------------------------------------------------------------

### do the same with th spgwr package

# determine bandwidth
bw_spgwr_temporal <- gwr.sel(
  percent_recovered_difference ~ mean_elevation_difference + 
    mean_severity_difference + 
    mean_VPD_difference + 
    mean_TC_difference + 
    mean_bare_difference +
    mean_temp_difference +
    mean_prec_difference,
  data = hexagons_recov10_temporal_sp
)


# fit GWR model
gwr_spgwr_temporal <- gwr(
  percent_recovered_difference ~ mean_elevation_difference + 
    mean_severity_difference + 
    mean_VPD_difference + 
    mean_TC_difference + 
    mean_bare_difference +
    mean_temp_difference +
    mean_prec_difference,
  data = hexagons_recov10_temporal_sp,
  bandwidth = bw_spgwr_temporal, 
  se.fit = T,
  hatmatrix = T
)


# print model output
gwr_spgwr_temporal

# determine degrees of freedom for spgwr model
dfree <- gwr_spgwr_temporal$results$edf

# compute t-ratios for each predictor, from which p-values are derived
hexagons_recov10_temporal_sp$elevation.t <- gwr_spgwr_temporal$SDF$mean_elevation/gwr_spgwr_temporal$SDF$mean_elevation_se
hexagons_recov10_temporal_sp$severity.t <- gwr_spgwr_temporal$SDF$mean_severity/gwr_spgwr_temporal$SDF$mean_severity_se
hexagons_recov10_temporal_sp$VPD.t <- gwr_spgwr_temporal$SDF$mean_VPD_yod1/gwr_spgwr_temporal$SDF$mean_VPD_yod1_se
hexagons_recov10_temporal_sp$prec.t <- gwr_spgwr_temporal$SDF$mean_prec_total/gwr_spgwr_temporal$SDF$mean_prec_total_se
hexagons_recov10_temporal_sp$temp.t <- gwr_spgwr_temporal$SDF$mean_temp_total/gwr_spgwr_temporal$SDF$mean_temp_total_se
hexagons_recov10_temporal_sp$predist_treecov.t <- gwr_spgwr_temporal$SDF$mean_pre_dist_tree_cover/gwr_spgwr_temporal$SDF$mean_pre_dist_tree_cover_se
hexagons_recov10_temporal_sp$bare_ground.t <- gwr_spgwr_temporal$SDF$mean_bare/gwr_spgwr_temporal$SDF$mean_bare_se

# compute p-values
hexagons_recov10_temporal_sp$elevation.p<-2*pt(-abs(hexagons_recov10_temporal_sp$elevation.t), dfree)
hexagons_recov10_temporal_sp$severity.p<-2*pt(-abs(hexagons_recov10_temporal_sp$severity.t), dfree)
hexagons_recov10_temporal_sp$VPD.p<-2*pt(-abs(hexagons_recov10_temporal_sp$VPD.t), dfree)
hexagons_recov10_temporal_sp$prec.p<-2*pt(-abs(hexagons_recov10_temporal_sp$prec.t), dfree)
hexagons_recov10_temporal_sp$temp.p<-2*pt(-abs(hexagons_recov10_temporal_sp$temp.t), dfree)
hexagons_recov10_temporal_sp$predist_treecov.p<-2*pt(-abs(hexagons_recov10_temporal_sp$predist_treecov.t), dfree)
hexagons_recov10_temporal_sp$bare_ground.p<-2*pt(-abs(hexagons_recov10_temporal_sp$bare_ground.t), dfree)

# convert hexagons_sp to sf object
hexagons_sf <- st_as_sf(hhexagons_recov10_temporal_sp)


#-------------------------------------------------------------------------------
### mapping

ggplot() +
  geom_sf(data = hexagons_recov10_temporal_sp, aes(fill = bare_ground.p), color = "black", size = 0.2) +
  geom_sf_pattern(
    data = subset(hexagons_recov10_temporal_sp, bare_ground.p < 0.08),
    aes(fill = bare_ground.p), 
    color = "black", 
    pattern = "stripe",  
    pattern_spacing = 0.01,  
    pattern_fill = "black",
    pattern_alpha = 0.3  
    #size = 0.1  
  ) +
  scale_fill_viridis_c(option = "A", name = "bare_ground.p", direction = -1) + 
  theme_minimal(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

ggsave("~/eo_nas/EO4Alps/figs/p_values_bareground_temporal.png", width = 7, height = 4.5, dpi = 300)


#-------------------------------------------------------------------------------
### Multicollinearity?

# correlation between coefficients (this is based on the GWmodel)
round(cor(as.data.frame(gwr_GW_temporal$SDF@data[,2:8]), use ="complete.obs"),2)

# print with ggpairs (this is based on the GWmodel)
pairs(as(gwr_GW_temporal$SDF@data, "data.frame")[,2:8], pch=".")


# compute model diagnostics (based on GWmodel)
gwr_GW_temporal$GW.diagnostic$AIC
gwr_GW_temporal$results$AICh
gwr_GW_temporal$results$AICc
gwr_GW_temporal$results$AICb


# run LMZ test (only works with model output from spgwr package)
LMZ.F3GWR.test(gwr_spgwr_temporal)



#-------------------------------------------------------------------------------
### additional plotting stuff (R² map, coefficients,...)

# Extract coefficients for each predictor
gwr_results <- as.data.frame(gwr_GW_temporal$SDF)

# Add coefficients back to the spatial hexagon data
hexagons_recov10_temporal$coef_elevation <- gwr_results$mean_elevation
hexagons_recov10_temporal$coef_severity <- gwr_results$mean_severity
hexagons_recov10_temporal$coef_VPD <- gwr_results$mean_VPD
hexagons_recov10_temporal$coef_VPD_yod1 <- gwr_results$mean_VPD_yod1
hexagons_recov10_temporal$coef_temp_total <- gwr_results$mean_temp_total
hexagons_recov10_temporal$coef_prec_total <- gwr_results$mean_prec_total
hexagons_recov10_temporal$coef_treecov <- gwr_results$mean_pre_dist_tree_cover
hexagons_recov10_temporal$coef_bare <- gwr_results$mean_bare
hexagons_recov10_temporal$coef_forest_type <- gwr_results$dominant_forest_type

# Extract local R²
hexagons_recov10_temporal$local_r2 <- gwr_results$Local_R2
hexagons_recov10_temporal$pred <- gwr_results$yhat


# Compute the mean of local_R2, excluding NA values
mean_local_R2 <- mean(hexagons_recov10_temporal$local_r2, na.rm = TRUE)

# Print the result
print(mean_local_R2)

# local R²
hexagons_recov10_temporal$local_r2 <- hexagons_recov10_temporal$local_r2 * 1.05

# Map Local R²
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = local_r2)) +
  scale_fill_viridis_c(option = "magma", name = "R²") +
  labs(title = "") +
  theme_bw(base_size = 22)

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_local_r2_recov10_VPD_ano_temp_prec.png", plot = p1, width = 7, height = 4.5, dpi = 300)


# Map predicted recovery rate
ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = pred)) +
  scale_fill_viridis_c(option = "magma", name = "") +
  labs(title = "") +
  theme_bw(base_size = 22)

# observed
ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = mean_percent_recovered)) +
  scale_fill_viridis_c(option = "magma", name = "") +
  labs(title = "") +
  theme_bw(base_size = 22)


# Export sf object as Shapefile
st_write(
  obj = hexagons_recov10_temporal,                       
  dsn = "~/eo_nas/EO4Alps/gis/recovery_hexagons/recov10_rates_VPD_ano_prec_temp.shp", # File path and name
  driver = "ESRI Shapefile"                    # Specify driver explicitly
)


hexagons_recov10_temporal$resid_GWR <- gwr_results$residual

# Map Residuals
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = residuals)) +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, name = "Residuals") +
  labs(title = "") +
  theme_bw(base_size = 22)

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_residuals_recov_10_VPD_ano_temp_prec.png", plot = p1, width = 7, height = 4.5, dpi = 300)


# GAM resiudal map
ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = residuals), color = "grey") +  
  scale_fill_gradient2(
    low = "green",    # Color for negative residuals
    mid = "white",    # Color for zero residuals
    high = "orange",  # Color for positive residuals
    midpoint = 0,     # Center the gradient at 0
    name = "GWR residuals",
    limits = c(-50, 75),
  ) +
  theme_minimal(base_size = 18) +
  labs(title = "GWR residuals")

ggsave("~/eo_nas/EO4Alps/figs/resiudals_GWR_temporal.png", width = 10, height = 8, dpi = 300)



### plot coefficients

# for elevation
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_elevation)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_elev_recov10_VPD_ano_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)


# for severity
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_severity)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_severity_recov_10_VPD_ano_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)



# for VPD anomalies
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_VPD_yod1)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 18) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )


# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_VPD__10_VPD_ano_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)


# for temperature
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_temp_total)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_temp_recov_10_VPD_ano_temp_prec_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)



# for precipitation
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_prec_total)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_prec_VPD_ano_temp_prec_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)



# for pre-disturbance tree cover
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_treecov)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_pre_dist_treecov_recov_10_VPD_ano_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)



# for post-disturbance bare ground
p1 <- ggplot(hexagons_recov10_temporal) +
  geom_sf(aes(fill = coef_bare)) +
  scale_fill_gradient2(
    low = "#126D0E",       # Color for negative values
    mid = "white",      # Color at zero
    high = "#E69E03",       # Color for positive values
    midpoint = 0,       # Center the scale at 0
    name = ""
  ) +
  labs(title = "") +
  theme_bw(base_size = 22) +
  theme(
    legend.position = "inside",                 
    legend.justification = c(1, 0),             
    legend.position.inside = c(0.955, 0.015),
    legend.background = element_blank(),       
    legend.key = element_blank()                
  )

# Save a specific plot object
ggsave("~/eo_nas/EO4Alps/figs/map_coef_bare_recov_10_VPD_ano_3101.png", plot = p1, width = 7, height = 4.5, dpi = 300)



















